...
1. **모델 준비.** 첫 실행 온보딩에서 **빠름 / 균형 / 정확** 프리셋 하나를 고르면 전사·번역 모델이 함께 준비됩니다. 하드웨어에 맞는 프리셋에 **추천** 배지가 붙고, 기기에 버거운 모델에는 **버거울 수 있음** 배지가 붙습니다. 설정 › 모델에서 언제든 프리셋을 바꾸거나 "직접 고르기"로 개별 모델을 고를 수 있습니다. 여러 모델을 눌러 두면 순서대로 내려받습니다.
...
Whisper 는 Hugging Face `ggerganov/whisper.cpp`의 GGML 모델, Qwen3-ASR 은 `ggml-org` 의 GGUF(본체 + 오디오 인코더 mmproj)를 앱 안에서 받습니다.
