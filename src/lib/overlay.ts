 ...
 * `tgt` 가 null 이면(번역 안 함) 또는 원어가 타겟과 같으면 번역이 오지 않으므로 기다리지 않는다.
 */
export function pairForOverlay(finals: Final[], tgt: string | null, now: number, lastFinalAt: number, waitMs = TRANSLATION_WAIT_MS): OverlayPair {
  ...
  const expected = tgt !== null && last.lang !== tgt;
  const prev = finals[finals.length - 2];
  if (expected && prev && now - lastFinalAt < waitMs) return { source: prev.text, translated: prev.tgt ?? "" };
