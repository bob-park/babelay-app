...
- Node/yarn 버전은 `.mise.toml`(node 24, yarn 4.18.0)이 결정한다. 셸에 mise가 활성화되어 있지 않으면 `mise exec -- yarn …`로 실행한다. `yarn --version`이 `4.18.0`이 아니면 진행하지 않는다.
...
yarn은 `.mise.toml`이 4.18.0로 고정한다. PnP 대신 node_modules 링커를 쓴다(Vite·sharp 호환).

```bash
npm pkg set packageManager=yarn@4.18.0
yarn --version
yarn config set nodeLinker node-modules
```

`packageManager` 필드는 corepack 셸이 남아 있는 환경에서도 yarn 1이 아닌 4.18.0을 고르게 한다.

Expected: `4.18.0`이 출력되고, 루트에 `.yarnrc.yml`(`nodeLinker: node-modules`)이 생긴다. 템플릿이 만든 `yarn.lock`(v1 형식)이 있으면 지우고 다음 단계의 `yarn add`가 새로 만들게 둔다.
