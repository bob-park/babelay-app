...
Post-phase UX change (2026-09-04, user runtime feedback): overlay shows only translated sets — new Finals do not replace the screen until their translation arrives; 3 s hold-then-source fallback replaced by a 15 s safety cap for failed/dropped translations (overlay.ts, spec §7.4, checklist). Ruling: keep a safety cap rather than hold forever — a failed or queue-dropped translation would otherwise freeze the overlay — cost if wrong: one source-only line after 15 s in failure cases.
